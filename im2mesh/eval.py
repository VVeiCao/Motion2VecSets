...
class MeshEvaluator(object):
    ...
    def eval_mesh(self, input_mesh, pointcloud_tgt, normals_tgt, points_iou, occ_tgt):
        ...
        if len(mesh.vertices) != 0 and len(mesh.faces) != 0:
            ...
            # print(occ[0:3], occ_tgt[0:3])
            # raise
            # print(occ.shape)
            # # calculate how many cells are the same
            # count_same = np.sum(occ == occ_tgt)
            # count_diff = np.sum(occ != occ_tgt)
    ...
    def eval_correspondences_mesh(self, meshes, pcl_tgt, project_to_final_mesh=False, return_colored_pcd=False):
    ...
    def vis_chamfer(self, meshes, pcl_tgt, project_to_final_mesh=False, return_colored_pcd=False):
        """Calculates correspondence score for meshes.
        ...
        for i in range(len(pcl_tgt)):
            v_t = deepcopy(meshes[i]).vertices
            pc_t = pcl_tgt[i][ind]
            v_t = np.asarray(v_t).astype(np.float32)
            pc_t = np.asarray(pc_t).astype(np.float32)
            completeness, completeness_normals = distance_p2p(
            pc_t, None, v_t, None)
            accuracy, accuracy_normals = distance_p2p(
            v_t, None, pc_t, None)

            chamfer_l1 = (completeness + accuracy) / 2.0

            chamfer_l1 = np.clip(chamfer_l1, 0, 0.04)
            chamfer_l1 = chamfer_l1 / 0.04
    ...
    def vis_chamfer_shape(self, mesh_t0, pcl_tgt, project_to_final_mesh=False, return_colored_pcd=False):
        """Calculates correspondence score for meshes.
