...
class Generator3D(object):
    ...
    def generate_from_latent(self, z, F, **kwargs):
        """
        F output a dist
        """
        self.implicit_F = F
        mesh = self.__generate_from_latent__(z, **kwargs)
        # try:
    ...
    def eval_points(self, p, z, c=None, **kwargs):
        ...
        for pi in p_split:
            ...
            with torch.no_grad():
                occ_hat = self.implicit_F(z, pi)

            occ_hats.append(occ_hat.flatten().detach().cpu())
