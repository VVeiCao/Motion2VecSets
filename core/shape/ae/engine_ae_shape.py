...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (data, meta) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        points = data['points'].to(device, non_blocking=True)
        labels = data['labels'].to(device, non_blocking=True)
        surface = data['surface'].to(device, non_blocking=True)
        
        size = points.shape[1] // 2
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            loss_vol = criterion(outputs[:, :size], labels[:, :size])
            loss_near = criterion(outputs[:, size:], labels[:, size:])
        ...
        pred = torch.zeros_like(outputs[:, :size])
        pred[outputs[:, :size]>=threshold] = 1

        accuracy = (pred==labels[:, :size]).float().sum(dim=1) / labels[:, :size].shape[1]
        accuracy = accuracy.mean()
        intersection = (pred * labels[:, :size]).sum(dim=1)
        union = (pred + labels[:, :size]).gt(0).sum(dim=1) + 1e-5
        ...
        if log_writer is not None and (data_iter_step + 1) % accum_iter == 0:
            ...
            log_writer.add_scalar('iou', iou, epoch_1000x)
...
# This evaluate is just an approximation
@torch.no_grad()
