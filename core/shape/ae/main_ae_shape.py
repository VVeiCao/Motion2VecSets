...
from engine_ae_shape import train_one_epoch, evaluate
...
def main(args):
    ...
    log_dir = f"output/{config_yaml.type}/shape/ae/{config_yaml.ae_model}/train_{formatted_date}"
    ...
    model = models_ae.__dict__['get_model'](N=config_yaml.pc_size, model=config_yaml.ae_model)
    model.to(device)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        misc.save_model(
            args=args, model=model, model_without_ddp=model_without_ddp, optiizer=optimizer,
            loss_scaler=loss_scaler, epoch=epoch, log_dir=log_dir, tmp_save = True)

        
        if log_dir and (epoch % args.save_every == 0 or epoch + 1 == args.epochs): # todo: change to 1
            misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch, log_dir=log_dir)

        if epoch != 0 and epoch % args.eval_every == 0 or epoch + 1 == args.epochs: # todo: change to 1
            test_stats = evaluate(data_loader_val, model, device)
            print(f"iou of the network on the {len(dataset_val)} test images: {test_stats['iou']:.3f}")
