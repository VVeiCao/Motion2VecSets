...
from engine_diff_shape import train_one_epoch, evaluate
...
def main(args):
    ...
    log_dir = f"output/{config_yaml.type}/shape/diff/{config_yaml.ae_model}_{config_yaml.diff_model}/train_{formatted_date}"
    ...
    ae = models_ae.__dict__[config_yaml.ae_model](N = config_yaml.pc_size)
    ae.eval()
    print("Loading autoencoder %s" % config_yaml.ae_model_path)
    ae.load_state_dict(torch.load(config_yaml.ae_model_path, map_location='cpu')['model'])
    
    ae.to(device)

    model = models_diff.__dict__[f"{config_yaml.ae_model}_{config_yaml.diff_model}"]()
    model.to(device)
    ...
    if args.eval:
        test_stats = evaluate(data_loader_val, model_without_ddp, ae, criterion, device, args)
