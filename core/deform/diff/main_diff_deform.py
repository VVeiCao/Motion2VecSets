...
from engine_diff_deform import train_one_epoch, evaluate
...
def main(args):
    ...
    appendix_timeattn = "_timeattn" if config_yaml.time_attn else ""
    appendix_corr = "_corr" if config_yaml.is_corr else ""
    log_dir = f"output/{config_yaml.type}/deform/diff/de_{config_yaml.deform_ae_model}_{config_yaml.diff_model}" + appendix_timeattn + appendix_corr
    ...
    ae = models_ae.__dict__[f"de_{config_yaml.deform_ae_model}"](N=config_yaml.pc_size)
    ae.eval()
    print("Loading autoencoder %s" % config_yaml.deform_ae_model_path)
    ae.load_state_dict(torch.load(config_yaml.deform_ae_model_path, map_location='cpu')['model'])
    ae.to(device)
    
    shape_ae = models_ae.__dict__[config_yaml.shape_ae_model](N=config_yaml.pc_size)
    print("Loading autoencoder %s" % config_yaml.shape_ae_model_path)
    shape_ae.load_state_dict(torch.load(config_yaml.shape_ae_model_path, map_location='cpu')['model'])
    shape_ae.to(device)
    shape_ae.eval()

    model = models_diff.__dict__[f"de_{config_yaml.deform_ae_model}_{config_yaml.diff_model}"](withT=config_yaml.time_attn, is_corr=config_yaml.is_corr)
    model.to(device)
    ...
    if args.lr is None:  # only base_lr is specified
        args.lr = args.blr * eff_batch_size / args.total_batch_size

    print("base lr: %.2e" % (args.lr * args.total_batch_size / eff_batch_size))
    ...
    criterion = models_diff.__dict__['EDMLoss']()
    ...
    if args.eval:
        test_stats = evaluate(data_loader_val, model, ae, criterion, device, args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, ae, shape_ae, criterion, data_loader_train,
        ...
        if log_dir and (epoch % args.save_every == 0 or epoch + 1 == args.epochs):
            misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch, log_dir=log_dir)

        if epoch % args.eval_every == 0 or epoch + 1 == args.epochs:
            test_stats = evaluate(data_loader_val, model, ae, shape_ae, criterion, device, args)
            print(f"loss of the network on the {len(dataset_val)} test images: {test_stats['loss']:.3f}")
