...
parser.add_argument('--points_nearsurface_size', type=int, default=100000,
                    help='Size of nearsurface points.')
parser.add_argument('--points_size', type=int, default=100000,
                    help='Size of uniform points.')


parser.add_argument('--face_idx_dir',  type=str, default='DFAUST_mesh',
                    help='Whether to save truth values as bit array.')
...
def process_path(in_path, args, face_idx_dict):
    ...
    with open(os.path.join(args.face_idx_dir, "face_idx_deform.json"), 'w') as f:
        json.dump({str(k): v.tolist() for k, v in face_idx_dict.items()}, f)
...
def export_pointcloud(modelname, model_files, args, face_idx_dict):
    ...
    if number in face_idx_dict.keys():
        face_idx = face_idx_dict[number]
    else:
        _, face_idx = mesh.sample(args.pointcloud_size, return_index=True)
        face_idx_dict[number] = face_idx
...
def export_nearsurface_points(modelname, model_files, args, face_idx_dict, sigma1=0.1, sigma2=0.02):
    ...
    if number in face_idx_dict.keys():
        ...
        _, face_idx = mesh.sample(args.points_nearsurface_size, return_index=True)
        face_idx_dict[number] = face_idx
